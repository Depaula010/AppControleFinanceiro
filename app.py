...
try:
    ...
    def processar_agendamentos():
        ...
        # Lança um erro para que o log do motor mostre a falha
...
try:
    # Prioridade para Linux (Render)
    locale.setlocale(locale.LC_ALL, 'pt_BR.UTF-8')
except locale.Error:
    try:
        # Fallback para Windows (Testes locais)
...
try:
    ...
    # Define fallbacks para evitar que o app crashe na inicialização
    GEMINI_API_KEY = None
    DATABASE_URL = None
    API_SECRET_KEY = 'uma-senha-bem-forte-12345'
    BOT_WHATSAPP_URL = 'https://bot-appfinanceiro-whatsapp.onrender.com'
...
def setup_database():
    ...
    # (DDL v12 completo, oculto por brevidade)
    sql_script_ddl = """
...
def populate_global_categories():
    ...
    # (DML global oculto por brevidade)
...
@app.route('/')
def home():
    return "API do Bot Financeiro v13 (Format R$ e Bug Fixes) está no ar!"

@app.route('/webhook-automate', methods=['POST'])
def handle_automate_webhook():
    """ 
    Rota do Gatilho Android (Notificações Automáticas) 
    *** CORRIGIDA: Sintaxe do 'except' e verificação de resposta vazia do Gemini ***
    """
