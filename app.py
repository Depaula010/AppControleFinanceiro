...
try:
    ...
    # A URL do nosso outro serviço (o Bot)
...
def setup_database():
    ...
    *** CORRIGIDO: tipo_transacao agora é VARCHAR(20) e inclui 'Pagamento Fatura' ***
    """
    ...
    CREATE TABLE Usuarios (
        ...
        api_key_automate VARCHAR(100) NULL UNIQUE,
    ...
    CREATE TABLE Transacoes ( 
        ...
        transferencia_par_id INT NULL REFERENCES Transacoes(id) ON DELETE SET NULL, 
        descricao VARCHAR(255) NOT NULL, 
        valor NUMERIC(15, 2) NOT NULL,
        data_transacao DATE NOT NULL DEFAULT CURRENT_DATE, 
        
        -- <<< CORREÇÃO AQUI: Aumentado para VARCHAR(20) e adicionado 'Pagamento Fatura'
        tipo_transacao VARCHAR(20) NOT NULL CHECK (tipo_transacao IN ('Renda', 'Despesa', 'Transferência', 'Pagamento Fatura')), 
    ...
    CREATE TABLE Agendamentos ( 
        ...
        conta_id INT NOT NULL REFERENCES Contas(id) ON DELETE CASCADE, 
        subcategoria_id INT NOT NULL REFERENCES SubCategoria(id) ON DELETE RESTRICT, 
        descricao VARCHAR(255) NOT NULL, valor_previsto NUMERIC(15, 2), 
    ...
    CREATE TABLE PotesDeGastos ( 
        ...
        periodicidade VARCHAR(20) NOT NULL DEFAULT 'MENSAL' CHECK (periodicidade IN ('SEMANAL', 'QUINZENAL', 'MENSAL', 'ANUAL')), 
    ...
    CREATE TABLE PoteSubCategorias ( 
        ...
        subcategoria_id INT NOT NULL REFERENCES SubCategoria(id) ON DELETE CASCADE, 
        UNIQUE(pote_id, subcategoria_id) 
    );
    
    -- Índices
    CREATE INDEX IF NOT EXISTS idx_usuarios_api_key_automate ON Usuarios(api_key_automate);
    ...
    CREATE INDEX IF NOT EXISTS idx_potes_usuario_id ON PotesDeGastos(usuario_id); 
    CREATE INDEX IF NOT EXISTS idx_pote_subcat_pote_id ON PoteSubCategorias(pote_id); 
    CREATE INDEX IF NOT EXISTS idx_pote_subcat_subcat_id ON PoteSubCategorias(subcategoria_id);
    """
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.commit() 
        return "Estrutura final do banco (v12 com VARCHAR(20)) criada/recriada com sucesso!", 200
...
def populate_global_categories():
    ...
    # *** CORREÇÃO: Removidos "BEGIN;" e "COMMIT;" do script ***
    ...
    -- 2.2. Inserir as Macro Categorias GLOBAIS
    ...
    -- 2.3. Inserir as Sub Categorias GLOBAIS
    ...
    -- 2.4. Inserção da Subcategoria "Outros" em TODAS as Macros GLOBAIS
    ...
    try:
        # *** CORREÇÃO: Gerenciamento explícito da transação ***
        with engine.connect() as conn:
            conn.begin() # Inicia a transação
            try:
                conn.execute(sql_populate_dml) 
                conn.commit() # Confirma se tudo deu certo
                return "Templates globais de categorias (v11) inseridos com sucesso!", 200
            except Exception as e_inner:
                print(f"Erro DENTRO da transação: {e_inner}")
                conn.rollback() # Desfaz se algo falhou
                raise e_inner # Joga o erro para o 'except' de fora
...
def setup_user_data():
    ...
    *** GERA E RETORNA SUA API KEY DO AUTOMATE ***
    ...
    # Gera uma nova API key segura para o Automate
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if user_data:
                ...
                # Se o ON CONFLICT não retornou (pode acontecer), busca manualmente
        ...
        print(f"Erro ao inserir dados do usuário: {e}") # Log do erro real
        try: 
            with engine.connect() as conn: 
                conn.rollback()
        except: 
...
def run_motor_agendamentos():
    """
    Esta é a rota secreta que o nosso Bot (index.js) vai chamar.
...
def handle_whatsapp_webhook():
    ...
    *** CORRIGIDA: Lógica de Pagamento de Fatura (SQL e Criação de Fatura) ***
    """
    if not engine or not model:
        return jsonify({"status": "erro", "mensagem": "Serviço não configurado"}), 503
    ...
    try:
        ...
        Classifique a intenção principal como "Renda", "Despesa", "Transferência" ou "Pagamento Fatura".
        ...
        with engine.connect() as conn:
            ...
            if intent == 'Renda' or intent == 'Despesa':
                ...
                # ... (Lógica de Renda/Despesa - Sem Mudança, Oculto por brevidade) ...
                prompt_extract = f"""..."""; response_extract = model.generate_content(prompt_extract); json_extract_text = response_extract.text.strip().replace("```json", "").replace("```", ""); transacao_gemini = json.loads(json_extract_text)
                ...
                prompt_categorize = f"""..."""; response_cat = model.generate_content(prompt_categorize); id_categoria_str = response_cat.text.strip().replace("`", "")
                try:
                    id_categoria_final = int(id_categoria_str)
                    if id_categoria_final not in [cat['id'] for cat in categories_json_list]: id_categoria_final = id_outros_fallback
                except ValueError: id_categoria_final = id_outros_fallback
            ...
            # --- FLUXO 4.B: SE FOR TRANSFERÊNCIA ---
                ...
                # ... (Lógica de Transferência - Sem Mudança, Oculto por brevidade) ...
                ...
                prompt_extract_transfer = f"""..."""; response_extract = model.generate_content(prompt_extract_transfer); json_extract_text = response_extract.text.strip().replace("```json", "").replace("```", ""); transfer_data = json.loads(json_extract_text)
                ...
                sql_get_subcat_transfer = text("SELECT s.id FROM SubCategoria s JOIN MacroCategoria m ON s.macro_id = m.id JOIN GrupoCategoria g ON m.grupo_id = g.id WHERE g.nome_grupo = 'Meta Financeira' AND s.nome_sub = 'Investimentos de Curto Prazo' AND s.usuario_id IS NULL LIMIT 1")
            ...
            # --- [CORRIGIDO!] FLUXO 4.C: SE FOR PAGAMENTO DE FATURA ---
                ...
                Extraia 3 coisas: "valor_decimal", "conta_origem" (de onde o R$ saiu), e "conta_cartao" (o cartão que foi pago).
                ...
                # --- CORREÇÃO (BUG 2): Encontra ou CRIA a fatura atual ---
                # Em vez de só procurar, usamos a mesma função do /webhook-automate
                fatura_id_pagar = get_or_create_fatura(conn, conta_id_cartao, data_hoje, usuario_id)
                
                # --- CORREÇÃO (BUG 1): Adiciona o JOIN GrupoCategoria g ---
                sql_get_subcat_pagto = text("""
                    SELECT s.id 
                    FROM SubCategoria s 
                ...
                if not conta_id_origem or not conta_id_cartao or not fatura_id_pagar or not id_subcat_pagto:
                    # Log mais detalhado
                    print(f"DEBUG: conta_id_origem: {conta_id_origem}, conta_id_cartao: {conta_id_cartao}, fatura_id_pagar: {fatura_id_pagar}, id_subcat_pagto: {id_subcat_pagto}")
                    raise Exception("Não foi possível encontrar as contas, uma fatura, ou a subcategoria de pagamento.")
