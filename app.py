...
from datetime import date, timedelta # Para calcular datas de fatura
...
def setup_database():
    """ Cria/Recria a ESTRUTURA final do banco com Faturas. Execute PRIMEIRO. """
    if not engine: return "Erro: Banco não configurado.", 500

    # SEU NOVO SCRIPT DDL (Parte 1 - Com Faturas)
    sql_script_ddl = """
    ...
    CREATE TABLE Faturas (
        ...
        status VARCHAR(20) NOT NULL DEFAULT 'Aberta' CHECK (status IN ('Aberta', 'Fechada', 'Paga')),
         UNIQUE(conta_id, data_vencimento) -- Só pode ter uma fatura por cartão/mês
    );
    ...
    CREATE TABLE Transacoes (
        ...
        fatura_id INT NULL REFERENCES Faturas(id) ON DELETE SET NULL, -- Novo FK
    ...
    -- Índices (incluindo o novo para fatura_id)
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.commit() 
        return "Estrutura final do banco (com Faturas) criada/recriada com sucesso!", 200
    except Exception as e:
        try: 
            with engine.connect() as conn: 
                conn.rollback() # CORRIGIDO: 'with' na linha seguinte, indentado
        except: 
...
def populate_global_categories():
    ...
    # SEU SCRIPT DML (Parte 2 - Templates Globais - IDÊNTICO AO ANTERIOR)
    # (Removido daqui por brevidade, mas o código completo está abaixo)
...
def setup_user_data():
    ...
    dia_vencimento_inter = 10 # Exemplo: Vence dia 10
    dia_fechamento_inter = 1  # Exemplo: Fecha dia 1
    ...
    sql_accounts = text(f"""
        ...
        ON CONFLICT (usuario_id, nome_conta) DO UPDATE SET -- Atualiza dias se a conta já existir
            dia_vencimento = EXCLUDED.dia_vencimento,
            dia_fechamento = EXCLUDED.dia_fechamento; 
    """)

    try:
        user_id = 1 # Assume ID 1 para simplificar
        with engine.connect() as conn:
            # Garante que o usuário existe
            result = conn.execute(sql_user)
            id_from_insert = result.scalar_one_or_none()
            if id_from_insert is None: # Se já existia, busca o ID
                 ...
                 user_id = id_from_insert

            # Insere/Atualiza contas (precisaria ajustar o ID se não fosse 1)
...
# ========= 3. ENDPOINTS PRINCIPAIS (LÓGICA DE FATURA!) =========

@app.route('/')
def home():
    return "API do Bot Financeiro v8 (Schema Final com Faturas) está no ar!"

def get_or_create_fatura(conn, conta_id, data_transacao):
    """
    Função auxiliar para encontrar ou criar a fatura correta para uma transação.
    """
    # 1. Buscar os dias de fechamento/vencimento do cartão
    sql_get_card_info = text("SELECT dia_fechamento, dia_vencimento FROM Contas WHERE id = :conta_id AND tipo_conta = 'Cartão de Crédito'")
    card_info = conn.execute(sql_get_card_info, {"conta_id": conta_id}).fetchone()
    
    if not card_info or not card_info.dia_fechamento or not card_info.dia_vencimento:
        # Não é um cartão de crédito configurado, ou falta informação
        return None # Indica que não deve ter fatura_id

    dia_fechamento = card_info.dia_fechamento
    dia_vencimento = card_info.dia_vencimento
    
    # 2. Calcular as datas da fatura ATUAL baseada na data da TRANSAÇÃO
    ano_transacao = data_transacao.year
    mes_transacao = data_transacao.month
    dia_transacao = data_transacao.day

    # Lógica de Fechamento/Vencimento (pode precisar de ajuste fino)
    if dia_transacao <= dia_fechamento:
        # Cai na fatura que fecha NESTE mês e vence no PRÓXIMO
        mes_fechamento = mes_transacao
        ano_fechamento = ano_transacao
        mes_vencimento = mes_transacao + 1
        ano_vencimento = ano_transacao
        if mes_vencimento > 12:
            mes_vencimento = 1
            ano_vencimento += 1
    else:
        # Cai na fatura que fecha no PRÓXIMO mês e vence no mês SEGUINTE
        mes_fechamento = mes_transacao + 1
        ano_fechamento = ano_transacao
    ...
    # Monta as datas completas (cuidado com dias inválidos como 31 em Fev)
    # Uma forma segura é pegar o primeiro dia do mês e adicionar dias
    try:
        data_fatura_fechamento = date(ano_fechamento, mes_fechamento, 1) + timedelta(days=dia_fechamento - 1)
        data_fatura_vencimento = date(ano_vencimento, mes_vencimento, 1) + timedelta(days=dia_vencimento - 1)
    except ValueError: # Dia inválido para o mês
         # Tenta usar o último dia do mês (simplificação)
         from calendar import monthrange
         _, ultimo_dia_fech = monthrange(ano_fechamento, mes_fechamento)
         _, ultimo_dia_venc = monthrange(ano_vencimento, mes_vencimento)
         data_fatura_fechamento = date(ano_fechamento, mes_fechamento, min(dia_fechamento, ultimo_dia_fech))
         data_fatura_vencimento = date(ano_vencimento, mes_vencimento, min(dia_vencimento, ultimo_dia_venc))

    # 3. Verifica se a fatura já existe
    ...
    if fatura_id is None:
        sql_create_fatura = text("""
            ...
            VALUES (:cid, :dv, :df, 'Aberta') RETURNING id
        """)
        ...
        fatura_id = result.scalar_one() # Pega o ID da fatura recém-criada
        print(f"[DATABASE] Nova fatura criada (ID: {fatura_id}) para cartão {conta_id} com vencimento em {data_fatura_vencimento}")
...
def handle_automate_webhook():
    ...
    try:
        ...
        usuario_id_fixo = 1 
        # !!! PRECISAMOS DE LÓGICA PARA IDENTIFICAR A CONTA !!!
        # Por enquanto, assumimos Banco Inter (ID 1) se for Renda, 
        # e Cartão Inter (ID 2) se for Despesa (MUITO SIMPLISTA!)
        conta_id_transacao = 1 if tipo_transacao_db == 'Renda' else 2 
        
        # Data da transação (pega a data de hoje)
        data_hoje = date.today()

        with engine.connect() as conn: # Abre a conexão uma vez
             ...
             sql_check_conta_tipo = text("SELECT tipo_conta FROM Contas WHERE id = :cid")
             conta_tipo_result = conn.execute(sql_check_conta_tipo, {"cid": conta_id_transacao}).scalar_one_or_none()
             
             if conta_tipo_result == 'Cartão de Crédito':
                 fatura_id_transacao = get_or_create_fatura(conn, conta_id_transacao, data_hoje)
                 if fatura_id_transacao is None:
                      print(f"AVISO: Não foi possível determinar/criar fatura para cartão {conta_id_transacao}, salvando sem fatura_id.")
                      # Decide se retorna erro ou salva sem fatura_id
                      # return jsonify({"status": "erro", "mensagem": "Erro ao processar fatura"}), 500
             ...
             conn.execute(sql_insert, {
                 ...
                 "fid": fatura_id_transacao, # Pode ser None se não for cartão
        ...
        # Tenta reverter se estava em uma transação
        try: 
