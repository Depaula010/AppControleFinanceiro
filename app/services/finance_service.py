...
def migrate_google_calendar_to_oauth_tokens():
    """
    Migra a tabela GoogleCalendarTokens para GoogleTokens e adiciona campos Gmail.

    Esta migração:
    1. Renomeia GoogleCalendarTokens → GoogleTokens
    2. Adiciona campos gmail_watch_expiration e gmail_history_id
    3. Cria índice para gmail_watch_expiration
    ...
    migration_sql = text("""
        -- Verificar se GoogleCalendarTokens existe
        DO $$
        BEGIN
            -- Renomear tabela se ainda não foi renomeada
            IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'googlecalendartokens') THEN
                ALTER TABLE GoogleCalendarTokens RENAME TO GoogleTokens;
                RAISE NOTICE 'Tabela renomeada: GoogleCalendarTokens → GoogleTokens';
            END IF;

            -- Adicionar coluna gmail_watch_expiration se não existir
            IF NOT EXISTS (
                SELECT FROM information_schema.columns
                WHERE table_name = 'googletokens' AND column_name = 'gmail_watch_expiration'
            ) THEN
                ALTER TABLE GoogleTokens ADD COLUMN gmail_watch_expiration BIGINT;
                RAISE NOTICE 'Coluna adicionada: gmail_watch_expiration';
            END IF;

            -- Adicionar coluna gmail_history_id se não existir
            IF NOT EXISTS (
                SELECT FROM information_schema.columns
                WHERE table_name = 'googletokens' AND column_name = 'gmail_history_id'
            ) THEN
                ALTER TABLE GoogleTokens ADD COLUMN gmail_history_id TEXT;
                RAISE NOTICE 'Coluna adicionada: gmail_history_id';
            END IF;
        END
        $$;

        -- Criar índice se não existir
        CREATE INDEX IF NOT EXISTS idx_google_tokens_watch_expiry
        ON GoogleTokens(gmail_watch_expiration);
    """)
    ...
    try:
        with db_engine.connect() as conn:
            ...
            conn.commit()
            print("[MIGRATION] ✅ Migração GoogleCalendarTokens → GoogleTokens concluída")
    except Exception as e:
