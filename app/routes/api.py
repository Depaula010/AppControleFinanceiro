...
@api_bp.route('/contas-mensais/<int:bill_id>', methods=['PUT', 'DELETE'])
@token_required
def manage_conta_mensal(user_id, bill_id):
    """
    Controla atualização e remoção de conta mensal (Alias em português).
    PUT: Atualiza
    DELETE: Remove (soft delete)
    """
    if request.method == 'PUT':
