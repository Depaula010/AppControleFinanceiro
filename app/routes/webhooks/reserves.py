...
"""
Rotas de gerenciamento de Reserva de Emergência.

Permite configurar quais agendamentos (contas fixas) devem ser incluídos
no cálculo da reserva de emergência ideal (6 meses de gastos essenciais).

Contém:
- toggle_incluir_reserva_agendamento: Alterna flag incluir_na_reserva
- listar_agendamentos_reserva: Lista agendamentos com filtros
"""
...
# Utilitários Fase A
from app.shared.decorators import handle_errors, validate_required_fields, require_user_auth
...
@handle_errors(tag="RESERVA_TOGGLE")
@validate_required_fields('incluir', 'api_key')
@require_user_auth
def toggle_incluir_reserva_agendamento(agendamento_id, usuario_id, numero_whatsapp_usuario):
    ...
    - @validate_required_fields: Validação automática de 'incluir' e 'api_key'
    - @require_user_auth: Autenticação automática
...
@handle_errors(tag="RESERVA_LIST")
