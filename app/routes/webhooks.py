        ...
        if not is_exclusive_event_word and (any(word in msg_upper for word in ['CONFIRMAR', 'OK', 'TROCAR', 'CANCELAR']) or msg_upper.isdigit()):
            ...
            if last_transaction_id:
                ...
                if status == 'saved':
                    ...
                    # NOVO: Se veio de email, marcar como processado
                    if dados.get('origem') == 'gmail_push' and dados.get('email_id'):
                        from app.services.gmail_label_service import GmailLabelService

                        email_id = dados['email_id']
                        usuario_id_email = dados['usuario_id']

                        print(f"[CONFIRM-SAVE] Marcando email {email_id} como processado")
                        GmailLabelService.mark_email_processed_with_retry(usuario_id_email, email_id)
...
def connect_calendar(usuario_id):
    ...
    try:
        ...
        if GoogleOAuthService.is_user_connected(usuario_id):
            return """
            <!DOCTYPE html>
            <html>
            <head>
        ...
        auth_url = GoogleOAuthService.get_authorization_url(usuario_id)
...
def oauth2callback():
    ...
    try:
        ...
        usuario_id = GoogleOAuthService.exchange_code_for_tokens(code, state)
...
def disconnect_calendar(usuario_id):
    ...
    try:
        ...
        GoogleOAuthService.revoke_access(usuario_id)
...
# ============================================
# GMAIL PUSH NOTIFICATIONS (PIX AUTOMATION)
# ============================================

def verify_pubsub_request():
    """
    Verifica se a requisição vem do Google Pub/Sub (JWT validation).
    ...
    if not auth_header.startswith('Bearer '):
        print("[GMAIL-PUSH] No Bearer token")
        return False
    ...
    try:
        ...
        # Extrair domínio base do redirect URI
        import urllib.parse
        ...
        print(f"[GMAIL-PUSH] ✅ JWT válido: {claim.get('email')}")
        return True

    except Exception as e:
        print(f"[GMAIL-PUSH] ❌ JWT inválido: {e}")
        return False

def get_user_by_gmail_address(email_address):
    """
    Busca usuário pelo endereço Gmail.

    Args:
        email_address: Email do Gmail (rafaeldepaulaextratos@gmail.com)

    Returns:
        tuple: (usuario_id, numero_whatsapp) ou None
    """
    from app.config import GMAIL_USER_EMAIL

    # Por enquanto, suportamos apenas um email configurado
    if email_address.lower() != GMAIL_USER_EMAIL.lower():
        return None

    # Buscar usuário no banco (assumindo que é o primeiro usuário)
    # Em produção, você pode ter uma tabela de mapeamento email → usuario_id
    sql = text("SELECT id, numero_whatsapp FROM Usuarios LIMIT 1")

    with db_engine.connect() as conn:
        result = conn.execute(sql).fetchone()
        if result:
            return (result.id, result.numero_whatsapp)

    return None

def get_last_processed_history_id(usuario_id):
    """
    Recupera último historyId processado para um usuário.

    Args:
        usuario_id: ID do usuário

    Returns:
        str: historyId ou None
    """
    sql = text("""
    ...
    with db_engine.connect() as conn:
        ...
        if result and result.gmail_history_id:
            return result.gmail_history_id

    return None

def update_last_history_id(usuario_id, history_id):
    """
    Atualiza último historyId processado.

    Args:
        usuario_id: ID do usuário
        history_id: Novo historyId
    """
    sql = text("""
        UPDATE GoogleTokens
        SET gmail_history_id = :hid
        WHERE usuario_id = :uid
    """)

    with db_engine.connect() as conn:
        conn.begin()
        conn.execute(sql, {"hid": history_id, "uid": usuario_id})
        conn.commit()

def mark_history_processed(history_id):
    """
    Marca historyId como processado no Redis (deduplicação).

    Args:
        history_id: historyId do Gmail
    """
    redis_key = f"gmail_history_processed:{history_id}"
...
def process_gmail_message(usuario_id, numero_whatsapp, message_id):
    """
    Processa um email individual do Gmail.

    Args:
        usuario_id: ID do usuário
        numero_whatsapp: WhatsApp do usuário
        message_id: Gmail message ID
    """
    from app.services.gmail_email_processing_service import GmailEmailProcessingService
    from app.services.gmail_label_service import GmailLabelService

    try:
        # 1. Verificar se já foi processado
        if redis_service.exists(f"gmail_processed:{message_id}"):
            print(f"[GMAIL-PUSH] Email {message_id} já processado")
            return

        # 2. Buscar email completo via Gmail API
        ...
        # 3. Extrair dados do email
        email_data = GmailEmailProcessingService.extract_email_data(message)

        # 4. Filtro 1: É Mercado Pago?
        if not GmailEmailProcessingService.is_mercado_pago_email(email_data):
            print(f"[GMAIL-PUSH] Email {message_id} não é Mercado Pago")
            redis_service.set_with_ttl(
                f"gmail_processed:{message_id}",
                {'skipped': 'not_mercadopago'},
                ttl_seconds=30 * 24 * 3600
            )
            return

        # 5. Filtro 2: É transação PIX?
        if not GmailEmailProcessingService.is_pix_transaction_email(email_data):
            print(f"[GMAIL-PUSH] Email {message_id} não é PIX")
            redis_service.set_with_ttl(
                f"gmail_processed:{message_id}",
                {'skipped': 'not_pix'},
                ttl_seconds=30 * 24 * 3600
        ...
        print(f"[GMAIL-PUSH] ✅ Processando email PIX: {message_id}")
        print(f"[GMAIL-PUSH] Assunto: {email_data['subject']}")

        # 6. Processar transação PIX (função definida abaixo)
        process_pix_email_transaction(
            usuario_id,
            numero_whatsapp,
            email_data,
            message_id
        ...
        print(f"[GMAIL-PUSH] ❌ Erro ao processar email {message_id}: {e}")
        import traceback
        traceback.print_exc()

@webhooks_bp.route('/webhook-gmail-pubsub', methods=['POST'])
def handle_gmail_pubsub_notification():
    """
    Recebe notificações Gmail Push do Google Cloud Pub/Sub.

    Fluxo:
    1. Gmail detecta novo email
    2. Gmail envia para Pub/Sub topic
    3. Pub/Sub push para este webhook
    4. Processamos email se for PIX do Mercado Pago
    """
    if not db_engine or not gemini_model:
        return jsonify({"status": "erro", "mensagem": "Serviço não configurado"}), 503

    try:
        # 1. Verificar autenticação Pub/Sub (JWT)
        if not verify_pubsub_request():
            print("[GMAIL-PUSH] ❌ Requisição não autorizada")
            return jsonify({"status": "erro", "mensagem": "Unauthorized"}), 401
        ...
        if not data or 'message' not in data:
            print("[GMAIL-PUSH] ❌ Formato inválido")
            return jsonify({"status": "erro", "mensagem": "Invalid format"}), 400
        ...
        if not email_address or not history_id:
            print("[GMAIL-PUSH] ❌ Dados faltando")
            return jsonify({"status": "erro", "mensagem": "Missing data"}), 400

        print(f"[GMAIL-PUSH] 📬 Notificação recebida: {email_address}, historyId: {history_id}")

        # 3. Verificar deduplicação
        if redis_service.exists(f"gmail_history_processed:{history_id}"):
            print(f"[GMAIL-PUSH] HistoryId {history_id} já processado")
            return jsonify({"status": "sucesso", "mensagem": "Already processed"}), 200

        # 4. Buscar usuário pelo email
        user_info = get_user_by_gmail_address(email_address)
        if not user_info:
            print(f"[GMAIL-PUSH] ❌ Usuário não encontrado para {email_address}")
            return jsonify({"status": "sucesso", "mensagem": "User not found"}), 200

        usuario_id, numero_whatsapp = user_info

        # 5. Buscar mudanças desde último historyId
        gmail_service = GoogleOAuthService.get_gmail_service(usuario_id)

        last_history_id = get_last_processed_history_id(usuario_id)
        ...
        if last_history_id:
            history_params['startHistoryId'] = last_history_id

        history = gmail_service.users().history().list(**history_params).execute()

        if 'history' not in history:
            print(f"[GMAIL-PUSH] Sem novos emails no history")
            mark_history_processed(history_id)
            update_last_history_id(usuario_id, history_id)
            return jsonify({"status": "sucesso", "mensagem": "No new messages"}), 200

        # 6. Processar cada novo email
        messages_processed = 0
        for history_record in history['history']:
            if 'messagesAdded' in history_record:
                for msg_record in history_record['messagesAdded']:
                    message_id = msg_record['message']['id']

                    print(f"[GMAIL-PUSH] Processando message: {message_id}")
                    process_gmail_message(usuario_id, numero_whatsapp, message_id)
        ...
        print(f"[GMAIL-PUSH] ✅ Processados {messages_processed} emails")

        return jsonify({
            "status": "sucesso",
            "mensagem": f"{messages_processed} emails processados"
        }), 200

    except Exception as e:
        print(f"[GMAIL-PUSH] ❌ ERRO: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({
            "status": "erro",
...
def process_pix_email_transaction(usuario_id, numero_whatsapp, email_data, message_id):
    """
    Processa transação PIX extraída de email Mercado Pago.
    Integra: Gemini extraction → Categorização → Confirmação WhatsApp → Save DB → Label email

    Args:
        usuario_id: ID do usuário
        numero_whatsapp: WhatsApp do usuário
        email_data: Dict com dados do email (extract_email_data)
        message_id: Gmail message ID
    """
    from app.services.gmail_label_service import GmailLabelService

    try:
        # 1. Extrair dados com Gemini AI
        print(f"[PIX-EMAIL] Extraindo dados com Gemini...")

        extracted = gemini_service.extract_from_email(
            email_data['subject'],
            email_data['body_text'] or email_data['body_html']
        )

        tipo_transacao = extracted['tipo_fluxo']  # "Renda" ou "Despesa"
        descricao = extracted['descricao_bruta']
        valor_decimal = float(extracted['valor_decimal'])
        data_hoje = date.today()

        print(f"[PIX-EMAIL] Tipo: {tipo_transacao}, Valor: R$ {valor_decimal:.2f}, Descrição: {descricao}")

        with db_engine.connect() as conn:
            # 2. Buscar contas do usuário
            contas_usuario = finance_service.get_user_accounts(conn, usuario_id)

            # PIX = débito imediato → Conta Corrente
            conta_id_transacao = next(
                (c[0] for c in contas_usuario if c[2] == 'Conta Corrente'),
                contas_usuario[0][0] if contas_usuario else None
            )

            if not conta_id_transacao:
                print(f"[PIX-EMAIL] ❌ Usuário sem contas cadastradas")
                return

            # 3. Buscar categorias e categorizar
            categories_json_list = finance_service.get_user_categories(
                conn, usuario_id, tipo_transacao
            )

            id_outros_fallback = finance_service.get_fallback_category_id(
                conn, tipo_transacao
            )
            ...
            print(f"[PIX-EMAIL] Categoria ID: {id_categoria_final}")

            # 4. Preparar dados da transação
            valor_para_db = valor_decimal if tipo_transacao == 'Renda' else (valor_decimal * -1)

            # Buscar nome da conta
            conta_nome = next((c[1] for c in contas_usuario if c[0] == conta_id_transacao), 'Conta Corrente')

            transacao_data = {
                'usuario_id': usuario_id,
                'conta_id': conta_id_transacao,
                'categoria_id': id_categoria_final,
                'fatura_id': None,  # PIX não é crédito
                'local': descricao,  # Ex: "LOJAS REDE"
                'descricao': None,   # Sem breakdown de itens para PIX
                'valor_db': valor_para_db,
                'valor_original': valor_decimal,
                'tipo_transacao': tipo_transacao,
                'tipo_pagamento': 'pix',
                ...
                # Metadata do email (para labeling posterior)
                'email_id': message_id,
                'email_thread_id': email_data.get('thread_id')
            }

            # 5. Verificar Redis disponível
            if not redis_service.is_connected():
                # Fallback: Salvar direto sem confirmação
                print("[PIX-EMAIL] ⚠️  Redis indisponível. Salvando direto...")

                finance_service.create_transaction(
                    conn, usuario_id, conta_id_transacao, id_categoria_final,
                    None, descricao, valor_para_db, tipo_transacao, data_hoje
                )
                conn.commit()
                ...
                GmailLabelService.mark_email_processed_with_retry(usuario_id, message_id)

                # Enviar notificação direta
                mensagem = f"✅ *PIX processado automaticamente*\n\n"
                mensagem += f"📍 {descricao}\n"
                mensagem += f"💵 {formatar_moeda(valor_decimal)}\n"
                mensagem += f"📊 Tipo: {tipo_transacao}\n"
                mensagem += f"🔔 Origem: Email automático"
            ...
            # 6. Criar transação pendente no Redis (confirmação em 2 etapas)
            transaction_id = TransactionConfirmationService.create_pending_transaction(
                numero_whatsapp,
                transacao_data
            )
            ...
            if not transaction_id:
                # Erro ao criar pendente → salvar direto
                print("[PIX-EMAIL] ⚠️  Erro Redis. Salvando direto...")
            ...
            # 7. Formatar mensagem de confirmação WhatsApp
            mensagem_confirmacao = TransactionConfirmationService.format_confirmation_message(
                transacao_data,
                categories_json_list,
                transaction_id
            ...
            # Adicionar indicador de origem (email)
            mensagem_confirmacao = mensagem_confirmacao.replace(
                '📂 Categoria:',
                '🔔 *Origem: Email PIX automático*\n📂 Categoria:'
            )

            # 8. Enviar confirmação por WhatsApp
            notification_service.enviar_notificacao_whatsapp(
                numero_whatsapp,
                mensagem_confirmacao,
                BOT_WHATSAPP_URL,
            ...
            # 9. Marcar email como em processamento
            redis_service.set_with_ttl(
                f"gmail_processed:{message_id}",
                {
                    'transaction_id': transaction_id,
            ...
            print(f"[PIX-EMAIL] ✅ Confirmação enviada. Transaction ID: {transaction_id}")

    except Exception as e:
        print(f"[PIX-EMAIL] ❌ Erro ao processar transação: {e}")
        import traceback
        traceback.print_exc()
