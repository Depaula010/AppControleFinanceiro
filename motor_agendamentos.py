    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            for ag in agendamentos:
                try:
                    conn.begin() # Inicia uma transação por agendamento
                    ...
                    # --- Lógica de Gasto Fixo ---
                        ...
                        if dia_hoje == ag.dia_execucao:
                            ...
                            sql_insert = text("""
                                ...
                                VALUES (:uid, :cid, :scid, :fid, :desc, :val, :tipo, :data)
                            """)
                            conn.execute(sql_insert, {
                                "uid": ag.usuario_id, "cid": ag.conta_id, "scid": ag.subcategoria_id,
                                "fid": fatura_id, "desc": ag.descricao, "val": ag.valor_previsto,
                    ...
                    # --- Lógica de Gasto Parcelado ---
                        ...
                        if dia_hoje == ag.dia_execucao and ag.parcelas_executadas < ag.total_parcelas:
                            ...
                            sql_insert_parc = text("""
                                INSERT INTO Transacoes (usuario_id, conta_id, subcategoria_id, fatura_id, descricao, valor, tipo_transacao, data_transacao)
                                VALUES (:uid, :cid, :scid, :fid, :desc, :val, :tipo, :data)
                            """)
                            conn.execute(sql_insert_parc, {
                                "uid": ag.usuario_id, "cid": ag.conta_id, "scid": ag.subcategoria_id,
                                "fid": fatura_id, "desc": descricao_parcela, "val": ag.valor_previsto,
                                "tipo": 'Despesa', "data": hoje
                            })
                            ...
                            conn.execute(sql_update_ag, {
                                "nova_parcela": nova_parcela_exec,
                                "novo_ativo": not desativar_agendamento,
                                "ag_id": ag.agendamento_id
                            })
...
# Ponto de Entrada (só para testes locais, não usado pelo Render)
if __name__ == "__main__":
    # Para testar, você precisa definir as variáveis de ambiente localmente
